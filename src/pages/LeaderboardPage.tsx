                  ...
                  >
                    ...
                    <div className="flex-1">
                      <p className={`font-bold text-sm ${isUser ? 'text-primary' : 'text-foreground'}`}>
                        ...
                        {player.badges?.includes('PFTO') && <span className="ml-1 text-xs bg-blue-500 text-white px-1 rounded">🎓</span>}
                        {player.badges?.includes('Café') && <span className="ml-1 text-xs bg-amber-700 text-white px-1 rounded">☕</span>}
                        {player.badges?.includes('Master') && <span className="ml-1 text-xs bg-purple-600 text-white px-1 rounded">🧙</span>}
                        {player.badges?.includes('CIBED') && <span className="ml-1 text-xs bg-green-600 text-white px-1 rounded">📚</span>}
                        {player.badges?.includes('Island') && <span className="ml-1 text-xs bg-cyan-500 text-white px-1 rounded">🏝️</span>}
                        {player.badges?.includes('Empreg') && <span className="ml-1 text-xs bg-gray-600 text-white px-1 rounded">💼</span>}
                        {player.badges?.includes('Pulta') && <span className="ml-1 text-xs bg-pink-500 text-white px-1 rounded">🎯</span>}
                        {player.badges?.includes('Condado') && <span className="ml-1 text-xs bg-red-400 text-white px-1 rounded">❤️</span>}
                      </p>
                    </div>
